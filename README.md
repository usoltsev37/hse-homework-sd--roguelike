...
* Управление персонажем осуществляется с помощью клашив со стелками или клавиш ```W```, ```A```, ```S```, ```D```
* Выход из игры осуществляется с помочью ```ESC```
* Открытие и закрытие инвенторя просходит по кнопки  ```I```
* По нажатию на ```SPACE``` происходит взаимодействие с предметами(их использование - атака для оружия)
* Кнопка ```ENTER``` выделяет текущий предмет в инвенторе 
* Кнопка ```R``` удаляет предмет из инвенторя
