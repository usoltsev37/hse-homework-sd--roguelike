...
В самом начале игрок имеет некоторое количество силы и здоровья. Игрок расположен в случайной клетке, причем остальные клетки он не видит из-за "тумана войны". При переходе по дорогом новые клетки появляются. В каждой такой клетке могут располагаться различные предметы(`$`), а также враги трех типов:
* `V` -- пассивный враг, не двигается, атакует только при нападении
* `Z` -- трусливый враг, убегает в сторону, противоположную от игрока
* `A` -- агрессивный враг, бежит в сторону игрока, чтобы немедленно атаковать
